...
def displace(source_path: Path, destination_path: Path):
    ...
    note_event_tracks = [
        (track_idx, track.note_events)
        for track_idx, track in get_note_tracks(midi_file)
    ]
    # (391, (145440, 'note_on', 48, 80))
    # (index, (cumulative time, note event, pitch, velocity))
    note_events = [
        (event, event_idx, track_idx)
        for track_idx, track in note_event_tracks
        for event_idx, event in track
    ]
        ...
        if msg_type == "note_on" and velocity > 0:
            if previous_time == time:
                ...
                if num_current_events > 1:
                    displacements = [
                        random.randint(displacement_min, displacement_max)
                        for _ in [0] * (num_current_events - 1)
                    ] + [0]
    ...
    for event, event_idx, track_idx in note_events:
        ...
        if displacement != 0:
            ...
            displacement_debt_per_track = [
                d + displacement for d in displacement_debt_per_track
            ]
            ...
            if displacement > 0:
                compensation_per_track = [
                    c + round(displacement / 8) for c in compensation_per_track
                ]

        debt = displacement_debt_per_track[track_idx]
        if debt > 0:
            click.echo(
                f"time_displacer track {track_idx} changing {curr_event.time} -> {curr_event.time + debt}"
            )
