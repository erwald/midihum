...
class MidihumTabular:
    ...
    def _save_model(self):
        click.echo(f"midihum_tabular saving model to {self.model_filename}")
        self.learn.export(self.model_filename)

    def _load_model_if_exists(self) -> bool:
        path = self.model_dir / self.model_filename
        if path.exists():
            click.echo(f"midihum_tabular loading model from {path}")
            self.learn = load_learner(path)
            return True
        click.echo(f"midihum_tabular couldn't find model at {path}")
        return False
    ...
    def _create_data_loaders(self) -> TabularDataLoaders:
        click.echo("midihum_tabular creating data loaders")

        self.train_df.velocity = preprocessing.minmax_scale(
            np.asfarray(self.train_df.velocity.values), feature_range=(-1, 1)
        )
        self.validate_df.velocity = preprocessing.minmax_scale(
            np.asfarray(self.validate_df.velocity.values), feature_range=(-1, 1)
        )
        ...
        return TabularDataLoaders.from_df(
            ...
            y_names="velocity",
            valid_idx=list(range(len(self.train_df), len(df))),
            bs=16,
        )

    def _create_learner(self, layers: Optional[List[int]], include_data: bool):
        click.echo(
            f"midihum_tabular creating learner with layers={layers} and include_data={include_data}"
        )
        ...
        self.learn = tabular_learner(
            dls=dls, layers=layers, y_range=(-1.2, 1.2), metrics=[rmse, exp_rmspe]
        )
    ...
    def train(
        ...
        click.echo(
            f"midihum_tabular training for {epochs} epochs with learning rate {lr} and weight decay {wd}"
        )
    ...
    def _rescale_predictions(preds: pd.Series) -> pd.Series:
        return (preds - preds.mean()) / (
            preds.std() * 2.5
        )  # not sure why 2.5, it just seems to work well ...
    ...
    def predict_validation_data(self, plot_dir: Path) -> pd.DataFrame:
        ...
        prediction_df["adjusted_error"] = (
            prediction_df.target - prediction_df.adjusted_prediction
        ).abs()
        ...
        for name in prediction_df.name.unique():
            ...
            click.echo(
                f"{name}: {correlation} (normal), {adjusted_correlation} (adjusted)"
            )
        total_adjusted_correlation = prediction_df.adjusted_prediction.corr(
            prediction_df.target
        )
        click.echo(
            f"total (adjusted) prediction-target correlation: {total_adjusted_correlation}"
        )
    ...
    def humanize(
        ...
        click.echo(
            f"midihum_tabular got {df.count()[0]} velocities in range ({min_velocity} ... {max_velocity})"
        )

        # load input midi file and, for each prediction, set the new velocity
        midi_file = MidiFile(source_path)
        velocities = [
            max(1, min(127, round(((row.prediction + 1.0) / 2.0) * 127.0)))
